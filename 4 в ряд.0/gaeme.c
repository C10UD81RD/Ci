...
void makeAIMove(char color, char board[ROWS][COLS])
{
    int col;
    srand(time(NULL));
    while (true)
...
int main()
{
    ...
    printf("Выберите цвет фишек (X/O): ");
    scanf("%c", &playerColor);
    if (playerColor == 'X')
        aiColor = 'O';
    else
    ...
    while (true)
    {
        ...
        {
            int col;
            printf("Ход игрока. Введите номер колонки (1-9): ");
            scanf("%d", &col);
            if (isValidMove(col, board))
            {
                makeMove(col, currentPlayer, board);
            ...
            {
                printf("Некорректный ход. Пожалуйста, повторите ввод.\n");
            }
        }
        else
        {
            printf("Ход компьютера.\n");
            makeAIMove(currentPlayer, board);
            displayBoard(board);
            if (checkWin(currentPlayer, board))
            {
